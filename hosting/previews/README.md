...
| **separate Vercel project** (projet `laligne`) | 🟡 **à déployer — King** — aperçu LA LIGNE OPTIC du 24/09 (nuit) : première page **dessinée** du dépôt (aucune photographie), le visage d'abord, choix des cinq formes de visage. ⚠️ **Déployer LE DOSSIER ENTIER** (`index.html` **+** `og.jpg`) ; **puis** `python3 demos/build_laligne.py --url https://<adresse>` et **redéployer** — `og:url`/`og:image` se remplissent là, pas avant (§20.7). Vérifier sur téléphone : les **cinq onglets de visage** doivent changer de panneau **même sans JavaScript**, et le dessin du premier écran se tracer. 0 capture ni mockup (pas de navigateur dans le bac). | **LA LIGNE OPTIC** (Mme Joungo Line Chantale), Akwa — `demo`, cabinet d'accord (« Ok merci beaucoup vyni » 21:04), aperçu construit, lien **pas encore envoyé** | `demos/concept-laligne-v1.html` ← gabarit `demos/laligne-v1.tpl.html` + `demos/build_laligne.py` |
