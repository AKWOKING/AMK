...
| **separate Vercel project** | ✅ **LIVE https://uni-labo.vercel.app** (déployé par King 18/09 22:05). **23/09 : le dossier `unilabo/` est entré dans `build_previews.py`** — il ne l'était pas, donc il restait sur la version du 18/09 pendant que le fichier canonique évoluait. La page a reçu depuis : le formulaire de réservation, la **fiche de prélèvement** (exemple, consigne, et **fiche vivante** qui se remplit), quatre photos « mise en situation », l'état d'ouverture calculé, la schéma `MedicalLaboratory` + `FAQPage` → **à redéployer**. ⚠️ **le dossier `unilabo/` porte maintenant un sous-dossier `img/` (4 photos) : déployer LE DOSSIER ENTIER, pas seulement `index.html`.** | **UNI-LABO**, Bonamoussadi Douala | `demos/concept-unilabo-v1.html` |
