...
|---|---|---|---|
| **separate Vercel project** (projet `dmoptic`) | 🟡 **à déployer — King** — construit le 24/09 au soir après la réponse « Ok Envoyé svp... » de M. Domche Noumbi (15:54). ⚠️ **Déployer LE DOSSIER ENTIER** (`index.html` **+** `og.jpg`) : sans la vignette, le lien arrive en carte grise. **Puis** `python3 demos/build_dmoptic.py --url https://<adresse>` et **redéployer** : `og:url`/`og:image` ne se devinent pas (§20.7). 0 capture ni mockup (pas de navigateur dans le bac) : l'œil de King sur un téléphone, en plein jour, avant d'envoyer. | **DM OPTIC** (M. Domche Noumbi), Douala — `demo`, aperçu construit, lien **pas encore envoyé** | `demos/concept-dmoptic-v1.html` ← gabarit `demos/dmoptic-v1.tpl.html` + `demos/build_dmoptic.py` |
