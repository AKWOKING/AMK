...
|---|---|---|---|
| **separate Vercel project** (projet `dmoptic`) | 🟡 **à redéployer — King** — **v2.1** (24/09, tard) : la page parle au patient, liste ses services et **montre les montures** (vue · soleil · enfants) ; le registre est sorti du texte visible. ⚠️ **Le lien https://dmoptic.vercel.app/ est en ligne mais porte encore la v1** — redéployer le dossier pour que la même adresse serve la v2.1. ⚠️ **Déployer LE DOSSIER ENTIER** (`index.html` **+** `og.jpg`) : sans la vignette, le lien arrive en carte grise. **Puis** `python3 demos/build_dmoptic.py --url https://<adresse>` et **redéployer** : `og:url`/`og:image` ne se devinent pas (§20.7). 0 capture ni mockup (pas de navigateur dans le bac) : l'œil de King sur un téléphone, en plein jour, avant d'envoyer. | **DM OPTIC** (M. Domche Noumbi), Douala — `demo`, aperçu construit, lien **pas encore envoyé** | `demos/concept-dmoptic-v1.html` ← gabarit `demos/dmoptic-v1.tpl.html` + `demos/build_dmoptic.py` |
