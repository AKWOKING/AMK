...
| Contactés | **47** | 31.5 % de la base |
| Réponses humaines | **4** | 8.5 % des contactés |
...
1. **Le volume contacté est le premier goulot** : 102 lead(s) sur 149 n'ont jamais reçu un message (68 % de la base). Aucune amélioration de texte ne compense un lead jamais contacté.
2. **Le taux de réponse humain** est de 8.5 % des contactés — c'est le chiffre à surveiller d'un envoi à l'autre (il se lit avec `SOURCES.md` : quelle liste répond).
3. **La conversion en rendez-vous, elle, ne fuit pas** : 3 des 4 réponses humaines ont donné un rendez-vous ou un prix posé. Le travail n'est donc pas de « mieux closer », il est de **contacter plus**, et de choisir les bonnes listes.
