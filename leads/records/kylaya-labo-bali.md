# Kylaya Labo

> ⚙️ **Fiche générée** par `leads/build/records.py` le 2026-09-21. Ne pas modifier à la main — les corrections vont dans `sales/Activity-Log.md`, et remontent ici au passage suivant.

## État (lu dans `leads/CRM.csv`)

| Champ | Valeur |
|---|---|
| Slug | kylaya-labo-bali |
| Type | lab |
| Ville | Douala (Bali, 189 Rue des Manguiers) |
| Langue de contact | FR |
| Étape | disqualified |
| WhatsApp | 696 78 77 78 |
| Numéro vérifié | no |
| Contacté | No |
| Réponse | No |
| Maquette / site | No |
| Source | directory |
| Détail source | annuaires de Douala |
| Site vérifié le | 2026-09-19 |
| Site existant | http://www.kylayalabo.com/ |

## Pourquoi il est écarté

A DÉJÀ UN SITE VIVANT (http://www.kylayalabo.com/). Hors cible — vérifié AVANT toute production (règle 66).

## Notes

⛔ NE PAS ENVOYER. A un site vivant. Écarté le 19/09 pendant la préparation du pack LABOS. C'est exactement le piège QUALITECH/Clinique des Anges — sauf que cette fois la vérification est passée avant la production, pas après.

## Prochaine action

**Aucune.** Lead écarté — A DÉJÀ UN SITE VIVANT (http://www.kylayalabo.com/). Hors cible — vérifié AVANT toute production (règle 66).

## Historique — lignes du journal qui citent ce lead

*Aucune ligne du journal ne cite encore ce lead. S'il a été contacté, c'est que l'envoi n'a pas été loggé — **à corriger dans `Activity-Log.md`**.*

---

## À la main (facultatif)

*Ce que tu écris ici est perdu au prochain passage du générateur. Pour garder une information, mets-la dans `sales/Activity-Log.md`.*
