...
r("", "", "", "", "")
r("TUE 15 SEP — DIGITAL-FIRST, CLINICS FIRST (King: clinics have more potential than schools). Plan = sales/Invitation-First-Replan-2026-09-15.md + sales/Clinic-Batch-2026-09-15.md. STANDING (King): every msg 1 goes WITH its mockup image (demos/shots/*-wa.jpg). DEPLOY: /clinic-bonaberi.html LIVE but partial-renamed (Buea eyebrow); /mitoc.html still 404 here; homepage stale — redeploy amk-site.zip for the fully consistent page (MITOC named link lives separately at mitoc-concept.vercel.app).", "", "", "", "")
r("", "", "", "", "")
...
r("⭐ Solidarity Clinic & Laboratory (Solidarity Health Foundation, Malingo/Molyko) — CLINIC #1",
  ...
  "WHATSAPP BLOCKED on front line: (1) quick WA-check admin 677 61 12 07 (SHF-CIG UN-registry number, unverified) -> if it answers as the clinic, attach demos/shots/mockup-clinic-wa.jpg FIRST then send the §1 nameless msg (live link held until redeploy clears /clinic-bonaberi.html 404); (2) if not, Solidarity moves to WED sealed-envelope sweep addressed to 'the current medical director or the clinic manager', never a personal name; (3) landline never cold-called; 691 63 29 41 Dr Njang personal = HOLD forever on Solidarity.", "☐", "Tue")
...
r("Tue clinics: MITOC SENT 08:35 (two ticks, awaiting; FU1 Thu 17). Solidarity: front line not on WA → check admin 677 61 12 07 (profile must answer as the clinic; attach demos/shots/mockup-clinic-wa.jpg) else Wed sealed card", "MITOC out; Solidarity routing open",
  "Yes → /clinic-bonaberi.html (LIVE but King's upload is a partial rename: Buea eyebrow/Molyko leftovers until he redeploys the canonical amk-site.zip; send with the placeholder caption; MITOC uses mitoc-concept.vercel.app) + clinic invite-choice ('10 minutes with the doctor Thu/Fri morning, or all here on WhatsApp?'); named concept within 24h; answers within 1h.", "Tue", "☐")
...
r("· REDEPLOY — KING'S ACTION (sandbox Vercel CLI is logged out): redeploy amk-site.zip (repo root, 13 files; smoke-tested all 200, every internal link resolves) to the EXISTING amk-cm.vercel.app project, NOT a new one. Live audit 15 Sep 13:30: /sample-secondary.html already new build; / stale clinic card + old thumbnail (fixed in bundle); /clinic-bonaberi.html and /mitoc.html 404 (in bundle). Verify after: open /clinic-bonaberi.html ('Healthcare that answers, day or night'), homepage clinic card opens it. No clinic nameless link shared until 404 clears; MITOC named link is the separate mitoc-concept.vercel.app.", "", "", "", "")
...
r("· Any 'yes send it' → sample link immediately (schools /sample-secondary.html LIVE; clinics /clinic-bonaberi.html is LIVE (partial-rename; placeholder caption until amk-site.zip redeploys), mockup-clinic-wa.jpg attached first); MITOC yes → mitoc-concept.vercel.app. Named concept request → AMK builds within 24h; move to kill list + book time.", "", "", "", "")
