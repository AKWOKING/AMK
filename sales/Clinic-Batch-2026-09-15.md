# Clinic Batch — Tuesday 15 Sep 2026 (King's priority sector)

> King, 14 Sep: clinics may have **more potential than schools**. This pair opens Tuesday before the school kill-list.
> Rules unchanged: WhatsApp-first, no cold calls, never sell in msg 1, ≤5 lines, verified pain only, no competitor names, no fake scarcity, BAMFAM, sign every message.
> King instruction, 14 Sep: **people rarely open email in this country** — no clinic plan depends on email. WhatsApp is the channel.
> Preview link used in this batch: **https://amk-cm.vercel.app/clinic-bonaberi.html** (nameless demo concept **Bonabéri Medical Centre, Douala** — renamed by King 15 Sep afternoon from Molyko Medical Centre; live via his manual upload, partial rename ("Buea" eyebrow + Molyko/MMC leftovers) until the canonical `site/clinic-bonaberi.html` ships on the next natural deploy. **King ruled 15:58: leave it as-is — "just a demo concept to show our work"; redeploy non-urgent, no Buea/neutral variant unless he later asks**; builder `site/build_sample_clinic.py`; old `sample-clinic.html` removed/404). Always send with the placeholder caption.
>
> **Solidarity is NOT messaged today (King confirmed 15:58 the reason: no direct WhatsApp line)** — front number 677 61 57 57 is voice-only; 691 63 29 41 = Dr Njang personal (forever HOLD here); admin 677 61 12 07 is only an optional 60-second profile check tonight, otherwise its first touch is **Wednesday's sealed-envelope card** ("current medical director or the clinic manager"). Not dropped — it rides the Wednesday sweep with the numberless lay schools.
> Owner: King sends all WhatsApp manually. AMK drafts only.

---

## 1. Solidarity Clinic & Laboratory (Solidarity Health Foundation) — Malingo/Molyko, Buea ⭐ BEST CLINIC BET

> **Research standard applied after King's 15 Sep correction; see `RESEARCH-STANDARD.md` (error log #1). The current medical director is NOT publicly identified. The message names NO doctor and routes through the gate.**

### Leadership — what is verified (and the stale-data warning)
- **Founder/CEO: Dr Peter Nde Fon (full name Peter Nde Fon)** — opened Solidarity Clinic in **August 1998**; GP (CUSS Yaounde 1983-1989), MPH from Germany, ex-Ministry of Health (1990-1998), lecturer and former **Chair of the Department of Public Health & Hygiene, Faculty of Health Sciences, University of Buea**, PC Molyko worshipper. **He DIED on 21 October 2020** in Douala (malaise; Douala General Hospital), buried at Alachu, Mankon. Sources: Cameroon-Info obituary, Mimi Mefo Info (13 Nov 2020). **Never name him as current staff; he is history, not a contact.**
- The clinic is operated by **Solidarity Health Foundation (SHF), a registered not-for-profit CIG** ("quality, accessible and affordable health care to vulnerable populations"; UN NGO Branch profile, P.O. Box 467 Buea, email nmbabit@gmail.com, listed phone **677 61 12 07** — `[UNVERIFIED]` admin line). His widow, **Madam Beatrice Nde Fon**, and the foundation staff were named as mourners/successors in the 2020 statement; family leadership is plausible but unconfirmed `[UNVERIFIED]`.
- **Current medical director: not found in public sources `[UNVERIFIED]` — the message must ask for them.**
- **Dr Asoba Anodem** — Medical Officer at Solidarity Health Foundation since **Sep 2011 to present** per his own LinkedIn `[VERIFIED-B, long-tenured staff, not necessarily decision-maker]`.
- **Dr Njang Mbeng Emmanuel (MD, MBA, MSc)** appears on medicoor's Solidarity doctor dropdown, but his own LinkedIn (last updated Apr 2025) lists: **District Medical Officer, Muyuka Health District (Oct 2020-present)**; **co-owner, One Stop Medical Laboratory & Diagnostics, Buea (May 2023-present)**; co-owner Africana Restaurant; President UB Medical School Alumni; CFO/co-founder HERO Cameroon; formerly GP Fundong District Hospital and Medical Director of Maealth (a mobile-health app company). **Solidarity is NOT in his listed experience**; the medicoor roster still shows the 2020-deceased founder too, so its staff data is stale `[STALE]`. Whether he still does sessions at Solidarity is **unconfirmed — DO NOT message him about Solidarity until King verifies he still works there** (he may instead be a prospect for his OWN lab; see sector notes).
- (Do NOT confuse any of the above with **Dr Peter Louis Ndifor**, proprietor of Bota Polyclinic Limbe and Vice President of the Cameroon Medical Council — a different living man; the 2019 WTWC PDF's "DR NDIFOR" was a truncated rendering of Nde Fon, the founder.)

### Numbers with attribution labels (research standard §5)
| Number | Label | Sources / notes |
|---|---|---|
| **+237 677 61 57 57** | **ORG-LIKELY clinic front line — NOT A WHATSAPP NUMBER (King-checked 15 Sep)** | Google Business Profile (Tier A), elidge, medicoor all carry it, but it is not registered on WhatsApp. Voice/SMS line only; per the no-cold-call rule it is not dialled. Demoted from WhatsApp send line. |
| **+237 677 61 12 07** | ORG-UNVERIFIED, SHF-CIG admin (UN NGO registry) | **Only remaining WhatsApp candidate: King to check whether its profile answers as Solidarity/the foundation.** If yes, msg 1 goes there; if no, Solidarity moves to Wednesday's sealed-card sweep. |
| **+237 691 63 29 41** | **PERSONAL-NAMED: Dr Njang's direct WhatsApp (King-confirmed ownership)** | Listed on medicoor against the clinic, but given his current roles and his own lab, treat as his personal line. **DO NOT TEXT a Solidarity pitch to it until his current relationship with the clinic is verified.** |
| +237 677 61 12 07 | UNKNOWN admin | UN NGO Branch SHF-CIG profile only; no corroboration. |
| +237 2 33 32 31 31 | Landline | medpages ("Solidarity Hospital", Molyko Checkpoint, D61). |

### Identity, location, footprint (sourced)
- **Google Business Profile: "Solidarity Health Foundation (Solidarity Clinic)"**, Untarred Malingo Street, Buea; plus code **575J+7M**; coordinates 4.1581816, 9.2816356 (medpages says Molyko Buea Checkpoint, road D61; P.O. Box 467).
- **4.0 stars / 54 Google reviews / 21 photos / open 24h** (checked live 15 Sep; King remembered 4.5 — it is 4.0). Review themes: "clean environment" (8), "professional staff" (8), caring staff/nurses/team, cardiologist, laboratory, pharmacy, free specialist consultations.
- **Google profile has NO website** ("Add website" button); no booking/services/prices attached. medicoor profile unclaimed and stale (still books against the late founder). Other footprint: elidge/medpages stubs, official **X account @SolidarityHeal1 (created May 2021, 789 followers, 269 posts — appears inactive recently)** `[VERIFIED-A]`, UN NGO Branch registration.
- Services per medicoor banner + reviews: 24h general medicine, vaccination, surgery, **laboratory** (hematology, parasitology, serology, microbiology, biochemistry, hormonology, water analysis), **maternity**, ultrasound/X-ray; reviews cite a cardiologist and an on-site pharmacy. Insurance: **Zenithe accredited list independently confirmed** ("SOLIDARITY HEALTH FOUNDATION, MOLYKO"); medicoor shows a wider insurer panel (verify before citing; "embassy approved" tags there are platform boilerplate — never repeat).
- Throughput credibility: peer-reviewed studies (Pan African Medical Journal 2023; BioMed Research International 2025) place Solidarity among the **three most-solicited facilities in the Buea Health District**; older malaria study (2016) lists "Solidarity Health Foundation Molyko" among the district's recognized facilities.
- Ignore solidarity-clinic.blogspot.com / .gr: an unrelated 2013 Athens, Greece social-enterprise template.

### Pain (verified; the pitch shape)
1. **A strong, living reputation (54 reviews, fresh reviews monthly, 24h) that dead-ends at phone calls** — no website, no WhatsApp booking, no services or prices attached to the Google profile that patients already trust.
2. A foundation mission ("affordable care for vulnerable populations", insurance acceptance, free specialist consults) invisible to donors/partners/patients; no channel to explain services in EN|FR.
3. Unclaimed/stale third-party profiles (medicoor still shows the founder who died in 2020) — the clinic does not control its own name online.
4. Lab results still require a physical trip; the 24h flow has no digital front door.

### Pitch (only after invitation)
Bilingual website attached TO the Google profile: 24h/emergency bar, six-service grid + lab bento, results-on-WhatsApp same day, transparent FCFA panels (their prices), maternity/ultrasound, insurance strip (Zenithe confirmed; more on their say-so), foundation/about page (mission, history since 1998 handled with respect, NOT trading on the late founder's name without family permission), EN|FR, bookings to the clinic WhatsApp with references, review-request flow to push 4.0 toward 4.5 organically. Profile-claim cleanup (medicoor) included. Budget sense: not-for-profit CIG, lead with patient-volume and grant/partner visibility value; if Dr Njang is later confirmed involved, his own lab (One Stop) becomes a separate commercial conversation, never mixed.

### Card fields
- What AMK knows: 24h not-for-profit clinic+lab+maternity(+pharmacy) on Malingo Street, founded 1998 by the late Dr Peter Nde Fon, run today by Solidarity Health Foundation (current director unknown; MO Dr Asoba Anodem since 2011); 4.0/54 Google reviews; Zenithe accredited; no website.
- One question: "Do patients currently receive lab results on WhatsApp, or must they travel back for them?"
- AMK brings: the nameless clinic concept on the phone; a sketched flow linking Google profile to site + WA booking.
- Visit conditions: current medical director OR clinic manager invited (name obtained at the gate/on chat); 24h named-preview offer; card never sells.

### Message 1 — WhatsApp (no personal names; ≤5 lines)
> **Channel status 15 Sep (King): the clinic line 677 61 57 57 is NOT on WhatsApp.** Send sequence: (1) WA-check admin line 677 61 12 07 — send below only if its profile answers as Solidarity/the foundation; (2) otherwise this exact text becomes the Wednesday sealed-envelope letter addressed to "the current medical director or the clinic manager"; (3) 691 63 29 41 never receives it, landline never cold-called.
> **Image (15 Sep, standing rule): on the WhatsApp leg attach `demos/shots/mockup-clinic.png` (nameless clinic mockup) with the text** — an image of the product beats description. The sealed-envelope leg carries the QR card as before. (MITOC already went out text-only per its own batch note; not retroactively changed.)
```
Good morning. Akwo King, AMK Web Development, Buea.
I build bilingual websites for private clinics and laboratories: services and lab panels listed, prices in FCFA, results and appointment requests on WhatsApp, English and French, ready in 3-5 days.
Your Google listing already has 54 patient reviews and 21 photos, but no website, services or booking link attached to it, so that goodwill ends at a phone call.
The image is a clinic-and-laboratory preview already built for exactly this. May I send the live version, and who should receive it, the current medical director or the clinic manager?
— Akwo King / AMK – Web Development & Digital Solutions
```
- FU1 M+2 / FU2 M+4 / FU3 M+7; warm yes → `/clinic-bonaberi.html` link → 24h named concept → invite choice.
- **691 63 29 41 (Dr Njang): HOLD.** Only if King verifies through the clinic itself (or Dr Njang confirms) that he still works there may he be messaged, and then with his dual role acknowledged. Separately qualified as his own lead below.
- Gatekeeper (nurse/front desk): ask, exactly like a school bursar, for the current director's own WhatsApp.

### Related prospect (do not merge)
- **One Stop Medical Laboratory & Diagnostics, Buea** — co-owned since May 2023 by **Dr Njang Mbeng Emmanuel** (also DMO Muyuka; built the Maealth "Help Yourself" health app; FAMSA/UBMSAA leader; his LinkedIn literally cites a "passion for community medicine and **digital driven healthcare solutions**"). High-fit digital-health buyer, likely entrepreneurial, but no footprint researched yet (no website confirmed). After Solidarity routing is resolved (and never via a Solidarity line), open a separate dossier; his direct WA is 691 63 29 41 per King. Status: `[LEAD-PARKED pending Solidarity resolution + research]`.

## 2. Midas Touch Optic Center (MITOC) — Molyko / Malingo, Buea

**Person in charge:** Ateasom Collins (name shown administering/associated with the Facebook page); role = proprietor/optician to confirm.
**Address:** Molyko, opposite former police station, Malingo, Buea (per the page's own intro).
**Phone:** +237 678 90 89 62 (Facebook page number — **King to verify on WhatsApp tonight**).
**Footprint:** Facebook page only (page id 100064126300520, ~365 likes, category Medical & health). Intro line: "We refract, prescribe n mount lenses". No website, no Google listing surfaced in search; phone books (mawidoO, business-guide P-O-C) carry the number with no other data.

### Verified facts
- **KING-VERIFIED 15 Sep 11:39 (Tier A, WhatsApp Business profile of 678 90 89 62):** the number IS the shop's own **business account named "MITOC"** (not a personal line); medical & health category; photo = the optician/proprietor in a white coat in front of the frame wall.
  - Business bio (their words): "Examine the eyes, prescribed and fit lenses. Also sells lens accessories and computer and swimming glasses." -> services/products now include **lens accessories, computer glasses and swimming glasses** (added to the named preview).
  - **Hours shown: 09:00-18:00** (days not stated; do not claim Sundays); address matches: "Opposite Former Police Station Malingo"; a **WhatsApp Catalogue is already in use** (they sell via catalogue) -> opening line acknowledges it.
  - Email shown: ateasomcollins@gmail.com (profile header renders "ateasoncollins"; ambiguous spelling, unused - no email pillar).
  - Named preview deployed by King at **https://mitoc-concept.vercel.app** (separate throwaway project for this one named lead; canonical copy stays site/mitoc.html inside the amk-cm project; named file kept OUT of the public sitemap).
- Services stated in the page's own words: refraction (eye testing), prescribing and mounting lenses — a full optician service, not just a frame shop.
- Facebook-only presence; the page carries no website link and no hours/price data in search-cached intro.
- Located in Molyko-Malingo, high-footfall student area; clientele naturally skews to students needing correction.

### Pain (verified)
1. Invisible on Google: a student with headaches searching "optician Buea" finds only Facebook, which requires the FB app/login and ranks poorly.
2. No online frame catalog or lens-price transparency; no way to book an eye test; appointment flow is "call/come".
3. Facebook pages offer no bilingual service/price structure and no WhatsApp booking button that lands as a chat on the owner's phone with a reference.

### Pitch (only after invitation)
- Clinic concept adapted to an optic center: eye-test booking on WhatsApp, frame collections gallery, lens option cards with FCFA prices, student-correction offer (his choice, no discounts invented), EN/FR toggle, emergency/repair CTA, map pin opposite the former police station.
- Smaller scope than Solidarity; frame catalog is the heavier content task (he supplies photos) → quote by scope, never discount.

### Card fields
- What AMK knows: optician at Molyko-Malingo opposite former police station; refraction, prescribing, lens mounting; Facebook-only; one mobile line.
- One question: "How many students walk out without booking because they cannot see your frames or prices beforehand?"
- AMK brings: clinic concept on phone + an optic-center re-skin sketch using the same template.
- Visit conditions: Mr Ateasom invited; 24h named preview offer; card does not sell.

### Message 1 — WhatsApp (≤5 lines) — POST-VERIFICATION FINAL 15 Sep 11:39 (send this)
> 678 90 89 62 confirmed as the MITOC business account. Send as one message, nothing attached.
```
Good morning. Akwo King, AMK Web Development, Buea.
I build bilingual websites for opticians: eye tests booked on WhatsApp, with frames, computer and swimming glasses and lens accessories priced in FCFA, in English and French.
I can see Midas Touch already keeps a WhatsApp catalogue, which is a great start. What is missing is a website and Google listing, so a student in Molyko searching "optician Buea" still cannot find or book you.
I have already prepared a free preview built around Midas Touch itself. May I send the link here?
— Akwo King / AMK – Web Development & Digital Solutions
```
- Changes vs the earlier draft and why: routing question removed (the business account answers the routing question); the WhatsApp catalogue is acknowledged (respects effort already made, and proves genuine research); two newly-verified product lines echo their own bio words; asks one trivial yes/no question ending the message.
- On YES: paste **https://mitoc-concept.vercel.app** with: "Thank you. It opens right on your phone, no app needed: try the EN/FR switch at top and press 'Book an eye exam' to see how a booking lands in your WhatsApp. Prices and frame photos are placeholders for yours. What would you change? I can walk you through it here, or show it in person in 10 minutes at the shop." (File fallback: attach site/mitoc.html as a document, 595 KB self-contained.)
- If they ask the price: "100 000 FCFA one-time: 50 000 to start and 50 000 when it is live. Your bilingual site, the frame gallery, WhatsApp booking and connecting your Google listing. I take two founding clients at these terms." Never discount; trade scope/timing.
- Silence: FU1 M+2 (Google search gap), FU2 M+4 (frame photos selling after 18:00, shared in faculty groups), FU3 M+7 (LATER/YES loop-close). After 3, stop.
- Claim discipline inside the preview: only refract / prescribe / mount lenses (their FB intro), real Malingo address; no hours, no brands, no contact lenses or surgery invented; prices tagged Demo; 9 generated concept photos, replaced by real shop shots at launch.

---

## Sector rule for the rest of the week
- **Research accuracy is now governed by `RESEARCH-STANDARD.md`**: source tiers, dated/labeled facts, mandatory named-person checks (collision + obituary + current-role + conflict), number attribution labels, and a per-message claim audit. No name or role from a directory enters a message without clearing it.
- Parks (do NOT pitch, already well digitized): Dr Liwo Dental Clinic (drliwo.org, polished site + blog + prices), Dr Soliman Clinic (solimanclinics.com, WordPress + online store + active socials, 4.8/5 on Google mirrors), St Veronica Medical Centre (biakahc.org, 24h site).
- Pattern to repeat when sourcing more clinics: **reachable WhatsApp + (no website OR a website-less Google profile with reviews) + named owner/doctor + a repeat-visit service** (lab results, refraction, scans, pharmacy refills) = qualified. A rich Google profile with no website link is the NEW highest-value pattern, as Solidarity proves.
- Every clinic concept is nameless first (`clinic-bonaberi.html`), named within 24h of a warm yes, built strictly per `AMK-DESIGN-SKILLS.md` (Forest palette family used for the nameless concept; rotate on named builds, never reuse OraCare's cream/navy Odentrics family).
