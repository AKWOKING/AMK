# Qualification MQL — 18 Sep 2026 (38 leads)

**Pre-flight :** `leads/leads_50.xlsx` (onglet *Leads 50*) · `sales/Pipeline-Status.md` · `sales/Clinic-Batch-2026-09-15.md` · `sales/Invitation-First-Replan-2026-09-15.md` · `leads/Daily Ops.csv`.
**Règle :** un lead n'entre en prospection que s'il **peut être joint** et s'il **n'a pas de site vivant**. Rien n'est déduit : ce qui n'est pas vérifié est marqué `?`, et un `?` ne compte pas comme un acquis.

## 1 · Les six portes MQL

| Porte | Question | Comment on la tranche |
|---|---|---|
| **M1 · Canal joignable** *(obligatoire)* | Peut-on écrire à quelqu'un ? | Numéro mobile **vérifié sur WhatsApp**, ou DM TikTok/FB actif, ou **invitation reçue** (carte scannée) |
| **M2 · Pas de site vivant** *(obligatoire)* | Y a-t-il quelque chose à vendre ? | Aucun site · expiré · cassé · annuaire seulement |
| **M3 · Douleur vérifiée** | Qu'est-ce qui leur coûte, de source publique ? | Résultats en PDF, pas d'admission en ligne, page inactive, avis non revendiqués |
| **M4 · Décideur identifiable** | Sait-on à qui on parle ? | Nom, ou rôle ciblable (« le principal », « le promoteur ») |
| **M5 · Capacité** | La taille justifie 100 000 FCFA ? | Effectifs, nombre de campus, années d'existence |
| **M6 · Disponible** | Est-ce le bon moment ? | Pas déjà servi · ou relance due |

> **CORRECTION 18/09 — M1 est ÉLIMINATOIRE.** Un lead sans canal vérifié n'est pas un prospect, quel que soit son score. Le score ne compense jamais l'absence de canal.
> **Correction 18/09 — M2 se dédouble.** « Pas de site » (bon signe : ils communiquent déjà) ≠ « aucune trace numérique » (mauvais signe : injoignables et indifférents depuis des années). Le second ne se prospecte pas.

**MQL = M1 ET M2 acquis, plus un total ≥ 4/6.** Sinon : `À VÉRIFIER` (il manque une information, pas un prospect), `NURTURE` (a déjà un site → vente plus tard), `PARK` (bloqué), `INBOUND` (il doit venir à nous).

## 2 · Le tableau

| # | Établissement | Ville | M1 | M2 | M3 | M4 | M5 | M6 | /6 | Verdict | Canal (tel que consigné) |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | COMOBIL – Collège Moderne Bilingue Les Lauréats | Douala | n | y | y | y | y | y | 5 | PARK — canal | N/V (ligne fixe) |
| 2 | Groupe Scolaire Moderne Bilingue WAFO | Douala | n | y | y | y | ? | y | 4 | PARK — canal | N/V |
| 3 | COSBINAL – Complexe Scolaire Bilingue NAL | Douala | ? | n | n | ? | y | y | 2 | NURTURE | probable (mobile) |
| 4 | Le Paradis des Anges (PDA) | Douala | ? | n | n | ? | y | y | 2 | NURTURE | 677 378 542 (probable) |
| 5 | American School of Douala (ASD) | Douala | n | n | n | ? | y | y | 2 | NURTURE | N/V |
| 6 | Divine Success Comprehensive College (DSCC) | Douala | **n** | y | ? | ? | y | y | 3 | **DISQUALIFIÉ** *(King 18/09 : 696 023 696 = « Kingdom Family Int'l », compte Finance)* | numéro faux |
| 7 | **PCSS Bonamoussadi** | Douala | ? | y | ? | ? | y | y | 3 | **À VÉRIFIER ★** | N/V |
| 8 | École Privée Bilingue Les Génies | Douala | ? | ? | ? | ? | y | y | 2 | À VÉRIFIER | N/V |
| 9 | Groupe Scolaire La Semence | Douala | ? | ? | ? | ? | ? | y | 1 | PARK | N/V |
| 10 | Institut Polyvalent Fosso | Douala | ? | ? | ? | ? | y | y | 2 | À VÉRIFIER | N/V |
| 11 | Collège Catholique Bilingue La Retraite | Yaoundé | ? | y | y | y | y | y | 5 | PARK — décision King | N/V |
| 12 | Complexe Scolaire et Universitaire Siantou | Yaoundé | ? | n | n | y | y | y | 3 | NURTURE | probable |
| 13 | Blessed Group of Schools (BGS) | Yaoundé | ? | n | n | ? | y | y | 2 | NURTURE | probable |
| 14 | Baptist High School (BHS) Awae | Yaoundé | **n** | y | ? | ? | y | y | 3 | **DISQUALIFIÉ** *(King 18/09 : 677 647 802 sans nom ni photo = non fiable)* | numéro faux |
| 15 | Rainforest International School (RFIS) | Yaoundé | n | n | n | ? | y | y | 2 | NURTURE | N/V |
| 16 | Institut Notre Dame des Apôtres | Yaoundé | ? | ? | ? | ? | y | y | 2 | À VÉRIFIER | N/V |
| 17 | St. Joseph's College Sasse (SJC Sasse) | Buea | y | y | y | y | y | y | 6 | INBOUND SEUL | 677 195 500 **pas sur WA** |
| 18 | PCSS Buea | Buea | y | y | y | y | y | n | 5 | SUIVI — envoyé | 652 075 229 = WA pro |
| 19 | Frankfils Comprehensive College | Buea | ? | y | ? | ? | y | y | 3 | À VÉRIFIER | N/V |
| 20 | Salvation Bilingual High School | Buea | y | y | ? | ? | y | y | 4 | INBOUND — carte | N/V |
| 21 | Marthlo Comprehensive Bilingual College | Buea | ? | y | ? | ? | y | y | 3 | À VÉRIFIER | N/V |
| 22 | Saint Anne's High School Limbe (SAHISCOL) | Limbe | ? | n | y | ? | y | y | 3 | PARK — site tenu | 334 745 678 à essayer |
| 23 | New Horizon International Comprehensive HS | Limbe | ? | n | n | y | y | y | 3 | NURTURE — upsell | 680 738 111 / 679 246 223 |
| 24 | College de l'Excellence de Limbe | Limbe | ? | y | y | ? | y | y | 4 | À VÉRIFIER | N/V |
| 25 | National Comprehensive High School (NCHS) | Limbe | ? | y | ? | ? | y | y | 3 | À VÉRIFIER | N/V |
| 26 | Presbyterian Girls Secondary School (PGSS) | Limbe | ? | ? | ? | ? | y | y | 2 | À VÉRIFIER | N/V |
| 27 | St. Theresa International Bilingual Comp. | Buea | y | y | y | y | y | n | 5 | INBOUND — octobre | confirmé WA |
| 28 | ~~Summerset Bilingual College~~ | Buea | **n** | y | y | y | y | y | 5 | **DISQUALIFIÉ — pas de canal** *(page FB dormante depuis 2015 = écarté par l'ICP D4 de King)* | aucun trouvé |
| 29 | ~~Saint Bernard High School~~ | Buea | **n** | y | y | y | y | ? | 4 | **DISQUALIFIÉ — pas de canal** *(aucune présence en ligne)* | aucun trouvé |
| 30 | ~~NABESK Comprehensive College~~ | Buea | **n** | y | y | y | ? | y | 4 | **DISQUALIFIÉ — pas de canal** *(aucune présence en ligne)* | aucun trouvé |
| 31 | Baird Memorial College | Buea | y | n | y | y | y | n | 4 | SUIVI — envoyé | confirmé WA |
| 32 | Baptist Comprehensive College | Buea | y | y | y | y | y | n | 5 | INBOUND SEUL | confirmé WA |
| 33 | St. Sylvester International College | Buea | y | y | ? | ? | y | y | 4 | INBOUND — carte | à vérifier |
| 34 | Bishop Jules Peters Memorial College | Buea | ? | y | ? | y | y | y | 4 | PARK — diocèse | « hold » |
| 35 | Inter Comprehensive High School (ICHS) | Buea | n | y | n | n | y | y | 3 | DISQUALIFIÉ | litige de propriété |
| 36 | Solidarity Health Foundation (clinique) | Buea | n | y | y | n | y | y | 4 | PARK — canal | 677 61 57 57 **pas sur WA** |
| 37 | Midas Touch Optic Center (MITOC) | Buea | y | n | y | y | y | n | 4 | SUIVI — envoyé | 678 90 89 62 = WA pro |
| 38 | One Stop Medical Laboratory & Diagnostics | Buea | n | ? | ? | y | y | y | 3 | HOLD — Dr Njang | 691 63 29 41 |

## 3 · Ce que la qualification dit vraiment

**Trois choses, et la troisième change le plan de la semaine.**

**① Il n'y a aucun MQL sortant disponible aujourd'hui.** Sur 38 leads, **6 atteignent 6/6** : Summerset, Saint Bernard, NABESK, Saint Sylvester, Salvation et SJC Sasse.
> **Correction du 18/09 (King) :** *aucune carte scellée n'a été déposée*. Ces six leads sont **froids, jamais approchés** — pas « invités ». Et **aucun d'eux n'a de numéro connu**, donc **aucun n'est joignable aujourd'hui**. La carte scellée reste une option à planifier ; elle n'est pas un contact existant.

**② Le vivier clinique est épuisé.** Les quatre cliniques restantes sont bloquées : Solidarity **n'a pas de numéro sur WhatsApp**, One Stop = **Dr Njang (interdit)**, MITOC et OraCare déjà servis. La règle « cliniques d'abord » n'a plus de carburant : la prochaine vague est scolaire, ou il faut **sourcer de nouvelles cliniques**.

**③ Le vrai goulot n'est pas la recherche, c'est la vérification des numéros.** Douze leads sont bloqués en `À VÉRIFIER` pour une seule raison : **personne n'a encore confirmé que leur numéro est sur WhatsApp**. Douala en compte cinq (Divine Success, PCSS Bonamoussadi, Les Génies, Fosso) et c'est là que se trouve la meilleure intention du fichier — **Divine Success fait déjà de la publicité TikTok** avec trois campus et aucun site.

→ **La tâche la plus rentable de la journée tient en dix minutes de téléphone** : vérifier les numéros ci-dessous. Chaque numéro confirmé ouvre un message ; c'est le seul geste qui augmente le nombre d'envois, et c'est exactement ce que demande le pivot.

## 4 · Liste de vérification WhatsApp (téléphone de King, ~10 min)

| Lead | Numéro à tester | Ce qu'on cherche |
|---|---|---|
| Divine Success (Douala, 3 campus) | mobile au CRM | profil qui répond au nom de l'école |
| PCSS Bonamoussadi (Douala) | mobile au CRM | idem |
| Institut Polyvalent Fosso (Douala) | mobile au CRM | idem |
| École Privée Bilingue Les Génies (Douala Akwa) | mobile au CRM | idem |
| COSBINAL (Douala) | mobile au CRM | idem |
| Baptist High School Awae (Yaoundé) | mobile au CRM | idem |
| College de l'Excellence de Limbe | mobile au CRM | idem |
| Frankfils / Marthlo / Salvation (Buea) | mobiles au CRM | idem |
| NCHS Limbe | mobile au CRM | idem |
| **Summerset · Saint Bernard · NABESK (Buea)** | — | pas de numéro trouvé : **c'est la carte scellée qui les fait venir**, on ne cherche pas à les joindre |

## 5 · Les trois premiers — et pourquoi eux

King a demandé de commencer par trois. Le tableau impose son ordre : les trois **6/6** de Buea — **Summerset, Saint Bernard, NABESK** — les seuls leads parfaits sur les six portes (canal mis à part). **Correction du 18/09 : ils sont froids et sans numéro**, donc leur préparation ne sert pas aujourd'hui — elle sert dès qu'un numéro est trouvé. Les messages sont prêts et **en anglais** (Buea est anglophone) dans [`Send-Pack-2026-09-18-1030.md`](Send-Pack-2026-09-18-1030.md).

Leurs dossiers complets (recherche, direction artistique, maquette téléphone + portable, texte) sont dans :

- [`clients/summerset/`](../clients/summerset/) — Summerset Bilingual College, Wokoko
- [`clients/saint-bernard/`](../clients/saint-bernard/) — Saint Bernard High School, Molyko
- [`clients/nabesk/`](../clients/nabesk/) — NABESK Comprehensive College, Bonduma
