...
**Impact (in repo 14 Sep):** `demos/sjc-sasse-v2.html` — King canonical file (123/123 EN|FR pairs, 383KB, real chapel photo base64 in hero): boarding, all-boys, oldest secondary school, 100% 2025 GCE, 8 dorms, 1,200-seat chapel, anthem, "Is Sasse a boarding school?" FAQ. AMK patch (`demos/patch_sasse_channels.py`, 14 Sep): admissions form now opens email to sajoscol@gmail.com (677 195 500 verified NOT on WhatsApp — a wa.me link there would dead-end); footer/contact gained landline 233 322 113, email, TikTok; reduced-motion block added. Old 24KB draft kept as `demos/sjc-sasse-homepage.html` (history).
...
**CONCEPT STATUS (14 Sep):** **`demos/concept-oracare-v3.html` is the canonical send file** — v2 Odentrics-reference build (clear FCFA prices) + the 24/7 EN|FR scripted assistant and emergency option merged from v1 (155/155 pairs). v1/v2 are history. First message SENT 14 Sep (video-vs-file double-bind); branch sheet + 30s video storyboard: `sales/Outreach-Pack-2026-09-14.md` §1. Host first under `/oracare/` (`hosting/previews/`).
