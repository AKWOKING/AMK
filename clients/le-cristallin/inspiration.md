...
3. **Le flyer ajoute un samedi 08h30–13h30 que le site ne montre pas** (lu sur l'image à l'écran, 21/09
   18:02), deux lignes de téléphone supplémentaires et une autre adresse e-mail. ⚠️ **Corrigé à 19:40 :**
   j'avais aussi inscrit « une adresse différente (Bonapriso / CTFIC Mballa 2) » — **sur l'image visible, le
   flyer porte lui aussi Akwa / FODEC / COMECI** : l'écart venait de mes notes, pas de ses supports. Donc
   **trois** divergences réelles, pas quatre, et la page ne fait plus que **demander** s'il existe un second
   local — elle ne corrige plus le client.
4. **Il a une page Facebook** — information donnée par lui dans sa note vocale de 18:01 (transcrite par King
   le 21/09 soir) et **absente de mes deux lectures** (ni le site, ni l'annuaire ne la signalaient) : ni l'un
   ni l'autre canal ne prend de rendez-vous. Il a demandé dans la foulée si nous voulions bien lui **en créer
   une autre** → le fil est passé de la modernisation à la **demande de prestation** (étape `closing`).
5. **Aucun prix** (leur choix, légitime : la monture dépend du modèle) et **aucune prise de rendez-vous**.
